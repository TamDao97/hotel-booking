  ...
  $(document).ready(function () {
    $(document).on("keyup change", "#search_form_input", function (event) {
      ...
      if (input_values.length > 0) {
        $("#search_suggestions_wrap, .search-suggestion-overlay").addClass(
          "show"
        );
      } else {
        $("#search_suggestions_wrap, .search-suggestion-overlay").removeClass(
          "show"
        );
      }
    });
    $(document).on(
      "click",
      ".search-suggestion-overlay, .search-click-icon",
      function () {
        $("#search_suggestions_wrap, .search-suggestion-overlay").removeClass(
          "show"
        );
    ...
    $(document).on(
      "click",
      ".suggestions-icon-close, .search-suggestion-overlay",
      function () {
    ...
    $(document).on(
      "click",
      ".shop-close-content-icon, .responsive-overlay",
      function () {
    ...
    $(document).on("click", ".pagination-list-item-next", function () {
      $(this)
        .parent()
        .find(".pagination-list-item.active")
        .next(".pagination-list-item")
        .addClass("active");
      $(this)
        .parent()
        .find(".pagination-list-item.active")
        .prev(".pagination-list-item")
        .removeClass("active");
    });
    ...
    $(document).on("click", ".single-shop-left-title .title", function (e) {
      ...
      if (shopTitle.hasClass("open")) {
        ...
        shopTitle
          .siblings(".single-shop-left-title")
          .children(".single-shop-left-inner")
          .slideUp(300, "swing");
    ...
    $(document).on("click", ".single-reservation-expandIcon", function (e) {
      ...
      if (shopTitle.hasClass("open")) {
        ...
        shopTitle
          .siblings(".single-reservation")
          .find(".single-reservation-inner")
          .slideUp(600);
    ...
    $(document).on("click", ".dashboard-list .has-children a", function (e) {
      ...
      if (sh.hasClass("open")) {
        sh.removeClass("open");
        sh.find(".submenu").children(".has-children").removeClass("open");
        ...
        sh.siblings()
          ...
          .removeClass("open");
        sh.siblings().find(".submenu").slideUp(300, "swing");
      }
    });
    $(document).on("click", ".close-bars, .body-overlay", function () {
      $(".dashboard-close, .dashboard-left-content, .body-overlay").removeClass(
        "active"
      );
    });
    $(document).on("click", ".sidebar-icon", function () {
      $(".dashboard-close, .dashboard-left-content, .body-overlay").toggleClass(
        "active"
      );
    });
    $(document).on("click", ".blog-details-side-title .title", function (e) {
      ...
      if (element.hasClass("open")) {
        ...
        element
          .siblings(".blog-details-side-title")
          .children(".blog-details-side-inner")
          .slideUp(300, "swing");
    ...
    $(".newsletter-area .container").length &&
      $(window).on("scroll", function () {
        ...
        })(".newsletter-area .container", 200)
          ? $(".newsletter-area .container").removeClass("container-fluid")
          : $(".newsletter-area .container").addClass("container-fluid");
      });
    ...
    if (globalSlickInit.length > 0) {
      $.each(globalSlickInit, function (index, value) {
        if ($(this).children("div").length > 1) {
          ...
          var lazyLoad =
            typeof allData.lazyload == "undefined" ? false : allData.lazyload;
          var appendDots =
          ...
          $(this).not(".slick-initialized").slick(sliderSettings);
        }
      });
    }
